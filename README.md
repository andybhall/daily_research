...
**https://andybhall.github.io/daily_research/** — a landing feed of dataset cards
(each a plain title + one-line description of the data, with a rubric sparkline and
score), a full page per memo leading with the title, a plain summary, a Willis-rubric
graph, and any data chart from that day's probe, and a rendered Ledger page. Each memo
carries `title` / `summary` front-matter that the site leads with; all charts are
inline SVG built from real, committed numbers — the generator never invents a data point.
